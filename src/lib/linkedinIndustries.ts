// Auto-generated from HarvestAPI's published LinkedIn industry taxonomy
// (https://github.com/HarvestAPI/linkedin-industry-codes-v2), id+label columns
// only. Used by resolveIndustryIds() below to translate an ICP's free-text
// industry names into the numeric industryIds the harvestapi/linkedin-
// company-search Apify actor's discovery filter actually requires — see
// src/lib/apify.ts's discoverCompaniesViaApify.
...
// Generic/filler words dropped before matching, so they can't produce noisy
// matches on their own (e.g. an ICP industry string like "Software / SaaS
// (general, cross-vertical)" shouldn't match every label that happens to
// also contain "and" or "of").
const STOPWORDS = new Set([
  "a", "an", "and", "the", "of", "for", "in", "on", "with", "to",
  "general", "cross", "vertical", "verticals", "other", "related", "based",
]);
...
const TOKENIZED_INDUSTRIES = LINKEDIN_INDUSTRIES.map((industry) => ({
  industry,
  tokens: tokenize(industry.label),
}));

// A word shared with only a handful of labels (e.g. "restaurants",
// "fintech"-adjacent terms) is real signal; a word shared with dozens of
// labels (e.g. "services", "technology", "industry") barely says anything
// on its own. Standard inverse-document-frequency weighting, "documents"
// being labels: rarer words score higher, common ones score lower — this
// is what actually distinguishes "Financial Technology" matching "Financial
// Services" (shares the rare, meaningful "financial") from it also
// matching "Space Research and Technology" (shares only the common,
// low-signal "technology") without hand-maintaining an ever-growing
// stopword list for every generic word the taxonomy happens to reuse.
const LABEL_COUNT_BY_WORD = new Map<string, number>();
for (const { tokens } of TOKENIZED_INDUSTRIES) {
  for (const word of tokens) {
    LABEL_COUNT_BY_WORD.set(word, (LABEL_COUNT_BY_WORD.get(word) ?? 0) + 1);
  }
}
const TOTAL_LABELS = LINKEDIN_INDUSTRIES.length;

function idfWeight(word: string): number {
  const count = LABEL_COUNT_BY_WORD.get(word) ?? 1;
  return Math.log(1 + TOTAL_LABELS / count);
}

// Two one-sided ratios, combined: how much of the QUERY's signal shows up
// in this label (recall), and how much of the LABEL's own name is
// explained by the query (precision) — each weighted by idfWeight so
// generic shared words count for less than distinctive ones. Recall alone
// isn't enough to tell "Financial Technology" -> "Financial Services" (a
// real match) apart from "Financial Technology" -> "Space Research and
// Technology" (coincidental): both share one query word ("financial" and
// "technology" respectively), giving them deceptively similar recall.
// Precision is what separates them — "Financial Services" is almost
// entirely explained by that one shared word (high precision, only
// "services" is left over), while "Space Research and Technology" has two
// other words ("space", "research") the query never mentioned (low
// precision). Combined via the harmonic mean (the same precision/recall
// trade-off as an F1 score), so a label only scores well when the match is
// substantial on BOTH sides, not just one.
function matchScore(queryTokens: Set<string>, labelTokens: Set<string>): number {
  let sharedWeight = 0;
  let queryWeight = 0;
  let labelWeight = 0;
  for (const word of queryTokens) {
    const weight = idfWeight(word);
    queryWeight += weight;
    if (labelTokens.has(word)) sharedWeight += weight;
  }
  for (const word of labelTokens) labelWeight += idfWeight(word);

  const recall = queryWeight > 0 ? sharedWeight / queryWeight : 0;
  const precision = labelWeight > 0 ? sharedWeight / labelWeight : 0;
  return recall + precision > 0 ? (2 * recall * precision) / (recall + precision) : 0;
}

// Below this, a match is more coincidental (one shared generic word,
// buried in an otherwise-unrelated label) than a real industry match —
// tuned against real cases: a genuine match like "Software / SaaS" ->
// "Software Development" scores well above this; a coincidental one like
// "Financial Technology" -> "Space Research and Technology" (shares only
// "technology", which explains almost none of that label's own name)
// scores well below it.
const MIN_MATCH_SCORE = 0.4;

// Translates an ICP's free-text industry names (e.g. "Software / SaaS
// (general, cross-vertical)") into real LinkedIn industryIds for Apify's
// harvestapi/linkedin-company-search discover filter — deterministic,
// code-owned matching rather than asking the model to recall or invent a
// numeric ID out of ~400 possible codes. Mirrors how `locations`/
// `companySize` are already just passed straight through: the model
// supplies natural language, code resolves it to what the actor actually
// needs.
//
// Returns [] when nothing clears MIN_MATCH_SCORE — callers treat that
// exactly like `locations`/`companySize` being absent (see
// discoverCompaniesViaApify), so an ICP describing an industry outside
// this taxonomy's vocabulary (or using a term the taxonomy just doesn't
// use, e.g. "fintech" vs. its own "Financial Technology") just falls back
// to keyword-only search, unchanged from before this filter existed.
export function resolveIndustryIds(industryNames: string[], maxIds = 8): number[] {
  const matched = new Map<number, number>(); // id -> best score seen

  for (const name of industryNames) {
    const queryTokens = tokenize(name);
    if (queryTokens.size === 0) continue;

    for (const { industry, tokens } of TOKENIZED_INDUSTRIES) {
      const score = matchScore(queryTokens, tokens);
      if (score < MIN_MATCH_SCORE) continue;
      const existing = matched.get(industry.id);
      if (existing === undefined || score > existing) {
        matched.set(industry.id, score);
      }
    }
  }
